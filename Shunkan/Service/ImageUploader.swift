...
//    
//    static func uploadChatsImage(image: UIImage, chatId:String,messageId:String,recipientId:String,recipientProfile:String,recipientName:String,senderId:String,senderUserName:String,metaData:StorageMetadata,storageChatRef:StorageReference) async throws -> String?{
//        guard let imageData = image.jpegData(compressionQuality: 0.5) else {return nil}
//        let filename = NSUUID().uuidString
//        let ref = Storage.storage().reference(withPath: "/chats/\(chatId)/\(filename)")
//        
//        do{
//            let _ = try await ref.putDataAsync(imageData)
//            let url = try await ref.downloadURL()
